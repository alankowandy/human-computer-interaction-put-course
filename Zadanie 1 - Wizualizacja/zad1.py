...
def plot_data(data, label, ax, markers, marker_positions):
...
def generate_plot():
    ...
    # Wczytanie jednego z wykresów do późniejszego wykorzystania przy atrybutach osi
    ...
    # Ustawienie pozycji markerów
    marker_positions = range(0, len(plot_attribute_data), 25)

    # Ustawienie typów markerów dla poszczególnych danych
    markers = {
        '1-Coev': 's',
        '1-Coev-RS': 'v',
        '1-Evol-RS': 'o',
    ...
    # Obliczenie średniej danych dla potrzeby posortowania
    mean_values = [data.iloc[:, 2:].mean(axis=1).mean() for data in loaded_plot_data]

    # Posortowanie danych i etykier na podstawie średniej
    ...
    for data, label in zip(sorted_plot_data, sorted_labels):
        ...
        plot_data(data, label, ax1, markers, marker_positions)
